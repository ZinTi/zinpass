...
namespace zinpass::repo {
    ...
    int InitDatabase::executeSQL(sqlite3* conn, const std::string& sql) {
        ...
        if (
            ...
            SQLITE_OK != rc)
        {
            utils::LogManager::add_log(std::string("SQL error: ") + std::string(errMsg) + std::string("\n"));
            sqlite3_free(errMsg);
            return rc;
        }
        return 0;
    ...
    int InitDatabase::init() const {
        ...
        if (const int rc = sqlite3_open(this->db_path_.c_str(), &conn)){
            utils::LogManager::add_log(std::string("[ERROR] Failed to open database: ") + std::string(sqlite3_errmsg(conn)));
            sqlite3_close(conn);
            return rc;
        }
        ...
        // 创建 metadata 表
        const std::string createMetadata =
        "CREATE TABLE metadata\n"
        "(\n"
        "    database_version INT NOT NULL,        -- 数据库版本号\n"
        "    daemon_version TEXT NOT NULL,         -- 创建该数据库的软件版本号\n"
        "    daemon_name TEXT,                     -- 应用名称\n"
        "    created_at TEXT NOT NULL,             -- 数据库创建时间\n"
        "    updated_at TEXT,                      -- 最后升级时间\n"
        "    description TEXT                      -- 描述\n"
        ");";
        executeSQL(conn, createMetadata);
        // 版本默认数据
        zinpass::utils::DateTime datetime_obj;
        const std::string now = datetime_obj.getCurrentDateTime();
        const std::string insertMetadata = "INSERT INTO metadata VALUES (1, '" ZINPASS_VERSION "', 'zinpassd', '" + now + "', NULL, 'Initial database schema');";
        executeSQL(conn, insertMetadata);
