...
void LoginDlg::refresh_combo_username() const {
    ...
    // 列出用户名
    const zinpass::UserRPC user_rpc(zinpass::ChannelManager::instance().get_channel());
...
void LoginDlg::on_btn_login_clicked() {
    auto user = zinpass::SystemUser(); // 用户
    // 获取用户名和输入框的内容
    ...
    zinpass::StateManager::instance().set_user_state(state);

    if(zinpass::StateManager::instance().get_user_state().session_id.size() == 36){
        e_key_->clear();
        accept(); // 登录成功，关闭登录对话框
    } else {
        std::string message;
