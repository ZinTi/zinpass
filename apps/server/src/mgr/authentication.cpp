...
namespace zinpass {
    AuthResult<std::optional<login_data_t>> Authentication::login_by_id(const std::string& id, const std::string& password) {
        ...
        if(const std::string hashed_password = zinpass::PwdUtils::digestPassword(password, data->getSalt());
        ...
        const unsigned int expires_seconds = static_cast<unsigned int>(zinpass::AppConfig::instance().get_session_timeout());
        const std::string session_id = session_mgr.create(data->getId(), "default data", expires_seconds);
        if (session_id.empty()) {
    ...
    AuthResult<std::optional<login_data_t>> Authentication::login_by_username(const std::string& username, const std::string& password) {
        ...
        // 1. 查询用户信息并验证用户身份合法性
        const auto [data, message] = zinpass::UserManager::find_by_username(username);
        if (!data) { // 查询用户失败
            return {std::nullopt, message};
        }
        ...
        // 2. 创建会话
        auto& session_mgr = zinpass::SessionMgr::instance();
        const int expires_seconds = static_cast<int>(zinpass::AppConfig::instance().get_session_timeout());
        const std::string session_id = session_mgr.create(data->getId(), "default data", expires_seconds);
        if (session_id.empty()) {
            return {std::nullopt, "会话建立失败"};
        }
    ...
    AuthResult<bool> Authentication::validate_user_by_id(const std::string& id, const std::string& password) {
        ...
        // 1. 查询用户
        const auto [data, message] = zinpass::UserManager::find_by_id(id);
        if (!data) {
