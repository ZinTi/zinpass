...
std::string SessionMgr::create(
        ...
        const unsigned int expires_seconds) {
    std::lock_guard<std::mutex> lock(mutex_);  // 线程安全锁

    std::string session_id = zinpass::UUIDGenerator::generate();
    const std::string now = zinpass::DateTime::getCurrentDateTime();
    const std::string expires_date_time = zinpass::DateTime::addSeconds(now, expires_seconds);

    const zinpass::Session new_session(
        session_id,
        sys_user_id,
        data,
        now,
...
void SessionMgr::refresh(const std::string& session_id, const int expires_seconds) {
    ...
    const std::string now = zinpass::DateTime::getCurrentDateTime();
    const std::string new_expires_date_time = zinpass::DateTime::addSeconds(now, expires_seconds);
...
bool SessionMgr::validate_session(const std::string& session_id){
    ...
    if(0 >= zinpass::DateTime::compareDateTimes(
        it->second.getExpiresTime(),
        zinpass::DateTime::getCurrentDateTime())
        ){
        return false;
    }
    return true;
...
int SessionMgr::purge_expired_sessions(void) {
    ...
    for (auto it = this->sessions_map_.begin(); it != this->sessions_map_.end(); ) {
        if (0 >= zinpass::DateTime::compareDateTimes(it->second.getExpiresTime(), now)) {
