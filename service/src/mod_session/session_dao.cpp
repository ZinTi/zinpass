...
namespace zinpass::mod_session {
    SessionDAO::SessionDAO() : pool_(repo::ConnectionPool::instance()) {
    }
    ...
    repo::DaoStatus SessionDAO::find_by_id(const std::string& id, mod_session::Session& session) const {
        ...
        if (SQLITE_ROW != sqlite3_step(stmt)) {
            utils::LogManager::add_log(std::string("[NOT FOUND] Session not found with the given ID in the database."));
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
        ...
        // utils::LogManager::add_log("[OK] Session found.");
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
    ...
    repo::DaoStatus SessionDAO::add(const mod_session::Session& session) const {
        ...
        if (SQLITE_DONE == result) {
            ...
            utils::LogManager::add_log("[OK] New Session created: " + session.getId());
        } else {
    ...
    repo::DaoStatus SessionDAO::update(const mod_session::Session& session) const {
        ...
        if (SQLITE_DONE != sqlite3_step(stmt)) {
            utils::LogManager::add_log(std::string("[ERROR] Failed to update session: ") + std::string(sqlite3_errmsg(conn)));
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
            return repo::DaoStatus::GenericError;
        }
        utils::LogManager::add_log(std::string("[OK] Session updated successfully: ") + session.getId());
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return repo::DaoStatus::Success;
    }
    ...
    repo::DaoStatus SessionDAO::remove(const std::string& id) const {
        ...
        utils::LogManager::add_log(std::string("[OK] Session removed successfully: ") + id);
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return repo::DaoStatus::Success;
    }
    ...
    repo::DaoStatus SessionDAO::update_expiration(const std::string& id, const std::string& expires_date_time) const{
        ...
        if (SQLITE_DONE != sqlite3_step(stmt)) {
            utils::LogManager::add_log(std::string("[ERROR] Failed to update session expiration: ") + std::string(sqlite3_errmsg(conn)));
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
            return repo::DaoStatus::GenericError;
        }
        utils::LogManager::add_log(std::string("[OK] Session expiration updated successfully: ") + id);
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return repo::DaoStatus::Success;
    }
