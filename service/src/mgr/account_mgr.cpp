...
AccountManager::Return<bool> AccountManager::delete_account(const std::string& account_id) {
    try {
        ...
        utils::LogManager::add_log(std::string("[ERROR] Invalid argument when deleting account: ") + e.what());
    } catch (const std::exception& e) {
        utils::LogManager::add_log(std::string("[ERROR] Exception when deleting account: ") + e.what());
    }
    return {false, "删除失败"};
}
