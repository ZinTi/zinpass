...
void DialogEditAccount::setup_ui(){
    ...
    lyt_grid_ = new QGridLayout; // 创建网格布局
    lyt_key_ = new QFormLayout; // 创建主密钥布局
    lyt_action_ = new QHBoxLayout;
...
void DialogEditAccount::initial_input_widgets() const {
    ...
    // 3. 渲染到 UI 控件
    edit_password_->clear(); // 清空密码输入框
    edit_main_key_->clear(); // 清空主密码输入框
...
void DialogEditAccount::on_btn_submit_clicked(){
    ...
    // 3. 更新记录
    const std::string session_id = zinpass::StateManager::instance().get_user_state().session_id;
    zinpass::AccountRPC account_rpc(zinpass::ChannelManager::instance().get_channel());
