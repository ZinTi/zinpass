...
gRPCClientService::~gRPCClientService() {
    ...
    utils::LogManager::add_log("[MESSAGE] gRPCClientService Destructor");
}

void gRPCClientService::run() const {
    utils::LogManager::add_log("[MESSAGE] Server listening on " + this->address_and_port_ );
    ...
    if (!server_) {
        utils::LogManager::add_log("[ERROR] Failed to start server on " + address_and_port_);
        return;
    }
    ...
    utils::LogManager::add_log("[MESSAGE] Server stopped on " + address_and_port_);
}
