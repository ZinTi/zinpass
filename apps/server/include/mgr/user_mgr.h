...
public:
     ...
     */
    static UserResult<std::optional<zinpass::SystemUser>> find_by_id(const std::string& id);
     ...
     * @return (查询到的用户数据 | null opt) + 成功与否 + 消息
     */
    static UserResult<std::optional<zinpass::SystemUser>> find_by_username(const std::string& username);
