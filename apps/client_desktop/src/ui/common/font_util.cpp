...
FontUtil::FontUtil() {
    ...
    // 创建默认代码字体
    code_font_ = QFontDatabase::systemFont(QFontDatabase::FixedFont);
    if (code_font_.family().isEmpty()) {
        code_font_.setFamily("Monospace");
    }
    // code_font_.setPointSize(12);
}
...
void FontUtil::loadApplicationFonts() {
    ...
    if (fontId != -1) {
        ...
        if (!families.isEmpty()) {
            // 更新单例中的字体
            FontUtil::instance().code_font_.setFamily(families.first());
        } else {
