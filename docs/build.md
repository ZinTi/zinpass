...
PowerShell cmd-let
```pwsh
Write-Output "" > $env:SystemRoot\zinpassd.bat # 创建/清空
notepad $env:SystemRoot\zinpassd.bat # 编辑
```

或 CMD DOS命令（效果同上）
```cmd
ECHO "" > %SystemRoot%\zinpassd.bat
notepad %SystemRoot%\zinpassd.bat
```

在其中编辑以下内容（依据实际路径修改）
```cmd
@echo off
"absolute\path\zinpassd.exe" %*
```
