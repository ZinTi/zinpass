...
void AccountMgrForm::setup_ui(){
    ...
    lyt_display_and_filter_ = new QHBoxLayout;
    lyt_bottom_btn_ = new QHBoxLayout;
...
void AccountMgrForm::list_accounts(){
    ...
    /*
    qDebug() << "提交数据:"
             << "\n(1)平台名:" << data["platform_name"].toString()
             << "\n(2)服务商:" << data["provider_name"].toString()
    ...
    for(int i = 0 ; i < num_rows ; ++i){
        zinpass::ViewAccount row = accounts[i];
    ...
    if(0 >= table_model_->rowCount()){
        ...
        coord_viewer_->setText("没有找到符合的记录或会话已失效");
    }else{
...
void AccountMgrForm::on_btn_add_account_clicked() const {
    dlg_add_acc_->clear_data(); // 新增数据前，将 dlg_add_acc_ 中的数据清除
    dlg_add_acc_->show();
}
