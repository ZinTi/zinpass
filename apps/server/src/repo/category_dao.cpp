...
CategoryDAO::CategoryDAO() : pool_(ConnectionPool::instance()) {
}
...
DaoStatus CategoryDAO::categoryNameToId(const std::string& categoryName, short* id) const {
    ...
    if (SQLITE_ROW == sqlite3_step(stmt)) {
        *id = static_cast<short>(sqlite3_column_int(stmt, 0));
        zinpass::LogManager::add_log("[OK] Category ID found.");
        status = DaoStatus::Success;
    }else {
        zinpass::LogManager::add_log(std::string("[NOT FOUND] Category ID not found with the given category name in the database."));
...
DaoStatus CategoryDAO::findCategories(std::vector<std::string>& categories) const {
    ...
    zinpass::LogManager::add_log(std::string("[OK] Number of category records found: ") + std::to_string(categories.size()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
