#include "account/account_filter_form.h"
#include "state/state_manager.h"
#include "state/channel_manager.h"
// RPC 封装
#include "rpc/account_rpc.h"
#include "rpc/phone_rpc.h"
#include "rpc/category_rpc.h"

AccountFilterForm::AccountFilterForm(QWidget *parent) : QWidget(parent){
    setup_ui();
    setup_layout();
    initial_input_widgets(); // 初始化输入控件
    initial_popup_window(); // 初始化下拉弹窗
}

AccountFilterForm::~AccountFilterForm(){
    if(this->popup_window_) { delete this->popup_window_; }
    if(box_filter_) { delete box_filter_; }
}


void AccountFilterForm::setup_ui(){
    // 创建控件
    box_filter_ = new QGroupBox("筛选条件", this);
    edit_platform_name_ = new QLineEdit(this);
    edit_provider_name_ = new QLineEdit(this);
    edit_username_ = new QLineEdit(this);
    edit_nickname_ = new QLineEdit(this);
    combo_phone_ = new QComboBox(this);
    combo_email_ = new QComboBox(this);
    combo_category_ = new QComboBox(this);
    edit_postscript_ = new QLineEdit(this);
    btn_refresh_ = new QPushButton("刷新", this);
    btn_submit_ = new QPushButton("查询", this);
    btn_drop_down_more_ = new QToolButton(this);
    //btn_drop_down_more_->setText("更多条件");
    btn_drop_down_more_->setPopupMode(QToolButton::InstantPopup); // 设置弹出模式
    btn_drop_down_more_->setArrowType(Qt::DownArrow); // 显示下拉箭头
    btn_drop_down_more_->setMinimumWidth(60);

    // 设置占位符文本
    edit_platform_name_->setPlaceholderText("平台名称");
    edit_provider_name_->setPlaceholderText("所属公司或组织");
    edit_username_->setPlaceholderText("用户名或用户ID");
    edit_nickname_->setPlaceholderText("昵称或姓名");
    edit_postscript_->setPlaceholderText("备注");

    // 统一设置控件高度
    constexpr int fixed_height = 24;
    edit_platform_name_->setFixedHeight(fixed_height);
    edit_provider_name_->setFixedHeight(fixed_height);
    edit_username_->setFixedHeight(fixed_height);
    edit_nickname_->setFixedHeight(fixed_height);
    combo_phone_->setFixedHeight(fixed_height);
    combo_email_->setFixedHeight(fixed_height);
    combo_category_->setFixedHeight(fixed_height);
    edit_postscript_->setFixedHeight(fixed_height);
}

void AccountFilterForm::setup_layout(){
    // 主布局
    lyt_main_ = new QHBoxLayout(this);
    lyt_gbox_ = new QHBoxLayout;

    // 表单布局
    lyt_form_left_ = new QFormLayout;
    lyt_form_left_->addRow("平台名", edit_platform_name_);
    lyt_form_left_->addRow("服务商", edit_provider_name_);
    lyt_form_left_->addRow("用户名", edit_username_);
    lyt_form_left_->addRow("昵称", edit_nickname_);
    //lyt_form_left_->setSpacing(6);

    lyt_form_right_ = new QFormLayout;
    lyt_form_right_->addRow("电话", combo_phone_);
    lyt_form_right_->addRow("邮箱", combo_email_);
    lyt_form_right_->addRow("类别", combo_category_);
    lyt_form_right_->addRow("备注", edit_postscript_);
    //lyt_form_right_->setSpacing(6);

    // 按钮布局
    lyt_btn_ = new QVBoxLayout;
    lyt_btn_->addWidget(btn_refresh_);
    lyt_btn_->addWidget(btn_submit_);
    lyt_btn_->addWidget(btn_drop_down_more_);
    lyt_btn_->addStretch(); // 拉伸

    // 表单对齐方式
    lyt_form_left_->setFormAlignment(Qt::AlignTop | Qt::AlignLeft);
    lyt_form_right_->setFormAlignment(Qt::AlignTop | Qt::AlignLeft);
    // lyt_btn_->setAlignment(Qt::AlignTop | Qt::AlignLeft);

    // 组合布局
    lyt_gbox_->addLayout(lyt_form_left_);
    lyt_gbox_->addSpacing(8);
    lyt_gbox_->addLayout(lyt_form_right_);
    lyt_gbox_->addSpacing(8);
    lyt_gbox_->addLayout(lyt_btn_);
    // lyt_gbox_->setContentsMargins(20, 20, 20, 20);
    box_filter_->setLayout(lyt_gbox_);
    lyt_main_->addWidget(box_filter_);

    // 连接信号槽
    connect(btn_refresh_, &QPushButton::clicked, this, &AccountFilterForm::on_btn_refresh_clicked);
    connect(btn_submit_, &QPushButton::clicked, this, &AccountFilterForm::form_submitted);
}

void AccountFilterForm::initial_popup_window(){
    this->popup_window_ = new PopupFilterWindow(this);
    // this->popup_window->setStyleSheet("background-color: lightgreen; color: #333333;");

           // 2. 处理下拉按钮点击事件
    connect(btn_drop_down_more_, &QToolButton::clicked, [this]() {
        if (this->popup_window_->isHidden()) {
            QPoint pos = mapToGlobal(btn_drop_down_more_->pos());
            pos.setY(pos.y() + btn_drop_down_more_->height());
            this->popup_window_->move(pos); // 移动到下拉按键的下方
            this->popup_window_->show();
            // 给窗口设置了Qt::Popup属性后无法输入中文，强制更新焦点链
            this->popup_window_->activateWindow(); //强制激活popup窗口（焦点窗口）
        } else {
            this->popup_window_->hide();
        }
    });

    //QObject::connect(btn_drop_down_more, &QToolButton::clicked, this, &AccountMgrForm::on_btn_drop_down_more_clicked); // 处理下拉按钮点击事件

}

void AccountFilterForm::initial_input_widgets() const {
    // 1. 定义变量
    const std::string session_id = zinpass::StateManager::instance().get_user_state().session_id;

    // 2. 获取数据：可选 phone_numbers、email_addresses、account_categories
    const auto channel = zinpass::ChannelManager::instance().get_channel();
    zinpass::PhoneRPC phone_rpc(channel);
    const auto[phone_numbers, message_phone] = phone_rpc.list_phone_numbers(session_id);
    zinpass::AccountRPC account_rpc(channel);
    const auto[email_addresses, message_email] = account_rpc.list_email_addresses(session_id);
    zinpass::CategoryRPC account_category_rpc(channel);
    const auto[name_of_categories, message_category] = account_category_rpc.list_account_categories(session_id);

    // 3. 渲染到 UI 控件
    combo_phone_->clear();
    combo_phone_->addItem(QString("所有")); // default
    combo_phone_->addItem(QString("空值(暂未实现)"));
    for(const std::string& phone_number : phone_numbers){
        combo_phone_->addItem(QString::fromStdString(phone_number));
    }

    combo_email_->clear();
    combo_email_->addItem(QString("所有"));
    combo_email_->addItem(QString("空值(暂未实现)"));
    for (const std::string& email_address : email_addresses) {
        combo_email_->addItem(QString::fromStdString(email_address));
    }

    combo_category_->clear();
    combo_category_->addItem(QString("所有"));
    combo_category_->addItem(QString("空值(暂未实现)"));
    for (const std::string& category : name_of_categories) {
        combo_category_->addItem(QString::fromStdString(category));
    }
}

void AccountFilterForm::on_btn_drop_down_more_clicked() const {
    if (this->popup_window_->isHidden()) {
        QPoint pos = mapToGlobal(btn_drop_down_more_->pos());
        pos.setY(pos.y() + btn_drop_down_more_->height());
        this->popup_window_->move(pos); // 移动到下拉按键的下方
        this->popup_window_->show();
        // 给窗口设置了Qt::Popup属性后无法输入中文，强制更新焦点链
        this->popup_window_->activateWindow(); //强制激活popup窗口（焦点窗口）
    } else {
        this->popup_window_->hide();
    }
}

void AccountFilterForm::on_btn_refresh_clicked() const {
    initial_input_widgets();     // 初始化控件
    edit_platform_name_->clear();
    edit_provider_name_->clear();
    edit_username_->clear();
    edit_nickname_->clear();
    edit_postscript_->clear();
}

QMap<QString, QVariant> AccountFilterForm::get_form_data() const{
    return {
        {"platform_name", edit_platform_name_->text()},
        {"provider_name", edit_provider_name_->text()},
        {"username", edit_username_->text()},
        {"nickname", edit_nickname_->text()},
        {"phone", combo_phone_->currentText()},
        {"email", combo_email_->currentText()},
        {"category", combo_category_->currentText()},
        {"postscript", edit_postscript_->text()}
    };
}
