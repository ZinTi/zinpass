...
grpc::Status AccountServiceImpl::ListEmailAddresses(ServerContext* context, const account::v1::ListEmailAddressesReq* request, account::v1::ListEmailAddressesResp* response) {
    ...
    const auto[email_addresses, message] = zinpass::AccountManager::get_email_options(user_id);
...
grpc::Status AccountServiceImpl::ListAccountById(ServerContext* context, const account::v1::ListAccountByIdReq* request, account::v1::ListAccountByIdResp* response) {
    ...
    const auto[data, message] = zinpass::AccountManager::get_account_by_id(account_id);
    response->set_message(message);
...
grpc::Status AccountServiceImpl::CreateAccount(ServerContext* context, const account::v1::CreateAccountReq* request, account::v1::CreateAccountResp* response){
    ...
    // 2. 验证主密钥是否正确，若错误则结束
    const auto[ret_validate, message_validate] = zinpass::Authentication::validate_user_by_id(user_id, request->main_key());
    if (!ret_validate) {
    ...
    const auto[ret_add, ret_message] = zinpass::AccountManager::add_view_account(main_key, view_account, request->password());
...
grpc::Status AccountServiceImpl::ListAccounts(ServerContext* context, const account::v1::ListAccountsReq* request, account::v1::ListAccountsResp* response) {
    ...
    std::string message;
    std::vector<zinpass::ViewAccount> accounts;
    ...
    std::string email_f;
    const auto ret = zinpass::AccountManager::getAccounts(
        message,
        provider_name_f,
        platform_name_f,
        username_f,
...
grpc::Status AccountServiceImpl::ListAccounts(ServerContext* context, const account::v1::ListAccountsReq* request, account::v1::ListAccountsResp* response) {
    ...
    const auto[v_accounts, message] = zinpass::AccountManager::get_accounts(
...
grpc::Status AccountServiceImpl::UpdateAccount(ServerContext *context, const account::v1::UpdateAccountReq *request, account::v1::UpdateAccountResp *response) {
    ...
    if (const std::string& new_passwd = request->password();
        ...
        // 检查主密钥是否正确
        if (const auto[ret_validate, msg_validate] = zinpass::Authentication::validate_user_by_id(user_id, request->main_key());
            !ret_validate) {
            response->set_result(false);
            response->set_message(msg_validate);
            return grpc::Status::OK;    // 主密钥不正确，中止更新
        }
        const auto[ret_ch_pwd, msg_ch_pwd] = zinpass::AccountManager::update_account_password(
    ...
    const auto[success, message_update] = zinpass::AccountManager::update_account_base_info(view_account);
...
grpc::Status AccountServiceImpl::DeleteAccount(ServerContext *context, const account::v1::DeleteAccountReq *request, account::v1::DeleteAccountResp *response) {
    ...
    // 3. 删除操作
    const std::string account_id = request->account_id();
    const auto[ret_delete, message_delete] = zinpass::AccountManager::delete_account(account_id);
...
grpc::Status AccountServiceImpl::FetchPassword(ServerContext* context, const account::v1::FetchPasswordReq* request, account::v1::FetchPasswordResp* response) {
    ...
    // 2. 验证主密钥是否正确
    const auto[result, message_validate] = zinpass::Authentication::validate_user_by_id(user_id, request->main_key());
    if (!result) {
    ...
    const std::string& account_id = request->account_id();
    const std::string& main_key = request->main_key();
    const auto[plaintext_password, message_plaintext] = zinpass::AccountManager::retrieve_decrypted_password(account_id, user_id, main_key);
