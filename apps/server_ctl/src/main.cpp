/**
 * zinpassctl 通过 gRPC 127.0.0.1:50052 与 zinpassd 交互
...
int main(int argc, char* argv[]){
    ...
    zinpass::CliCommand::version(true);
    std::cout << "Enter \"help\" for usage hints." << std::endl;

    // 获取单例实例
    auto& prompt = zinpass::Prompt::instance();

    zinpass::ReadEvalPrintLoop REPL;
    REPL.run();
    return 0;
}
...
#endif
    zinpass::ChannelManager::instance().initialize("127.0.0.1:50052");    // 初始化通道管理器
}