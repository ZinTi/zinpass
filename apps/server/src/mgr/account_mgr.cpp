...
std::vector<unsigned char> AccountManager::main_key_to_real_key(const std::string& main_key){
    const std::string realKeyHex = zinpass::PwdUtils::calculateMD5(main_key);
    const std::vector<unsigned char> realKey = zinpass::PwdUtils::hexStringToKey(realKeyHex);
    return realKey;
}
...
AccountManager::Return<std::vector<std::string>> AccountManager::get_email_options(const std::string& sys_user_id) {
    ...
    std::string message;
    try {
        zinpass::AccountDAO email_dao;
        if (const zinpass::DaoStatus dao_ret = email_dao.findEmailList(sys_user_id, email_addresses);
            dao_ret == zinpass::DaoStatus::Success)
        {
            message = "成功";
        } else if (dao_ret == zinpass::DaoStatus::NotFound) {
            message = "未查询到";
        } else {
            message = "发生错误";
        }
...
AccountManager::Return<std::vector<std::string>> AccountManager::get_category_options() {
    ...
    std::string message;
    try {
        zinpass::CategoryDAO category_dao;
        if (const zinpass::DaoStatus dao_ret = category_dao.findCategories(categories);
            dao_ret == zinpass::DaoStatus::Success)
        {
            message = "成功";
        } else if (dao_ret == zinpass::DaoStatus::NotFound) {
...
AccountManager::Return<std::vector<zinpass::ViewAccount>> AccountManager::get_accounts(
    ...
    std::vector<zinpass::ViewAccount> view_accounts;
    std::string message;
    try {
        zinpass::AccountDAO account_dao;
        if (const zinpass::DaoStatus dao_ret = account_dao.find(provider_name,platform_name,
...
AccountManager::Return<zinpass::ViewAccount> AccountManager::get_account_by_id(const std::string& account_id) {
    zinpass::ViewAccount view_account;
    std::string message;
    const zinpass::AccountDAO account_dao;
    if (const zinpass::DaoStatus ret = account_dao.findById(account_id, view_account);
        zinpass::DaoStatus::Success != ret
        ) {
...
AccountManager::Return<std::string> AccountManager::retrieve_decrypted_password(const std::string& account_id, const std::string& sys_user_id, const std::string& main_key) {
    ...
    if(std::string plaintext;
        zinpass::PwdUtils::decryptTextByAES128CTR(ciphertext, iv, main_key_to_real_key(main_key), plaintext))
    {
        ret.data = plaintext;
        ret.message = "成功";
    }else{
...
AccountManager::Return<bool> AccountManager::add_account(
    ...
    const bool ret_encrypt = zinpass::PwdUtils::encryptTextByAES128CTR(
        password,                               // 原明文密码
    ...
    if (phone != "无" && phone != "空" && phone != "NULL") {
        const zinpass::MobilePhoneDAO phone_dao;
    ...
    if (email != "无" && email != "空" && email != "NULL") {
        const zinpass::AccountDAO email_dao;
        email_id = email_dao.getIdByEmailAddress(email);
    }

    const zinpass::CategoryDAO category_dao;
    short temp_category_id;
    if (const zinpass::DaoStatus ret_category_dao = category_dao.categoryNameToId(category, &temp_category_id);
        zinpass::DaoStatus::Success == ret_category_dao) {
        category_id = temp_category_id;
    }

    const std::string current_date_time = zinpass::DateTime::getCurrentDateTime();

    // 4. 创建对象并写入到数据库中
    const zinpass::Account account(
        "",                  // 底层函数会自动分配主键
        username,           // 参数
        nickname,           // 参数
        sub_account,        // 参数
    ...
    const zinpass::AccountDAO account_dao;
    if (zinpass::DaoStatus ret_account_dao = account_dao.add(account);
        zinpass::DaoStatus::Success != ret_account_dao
        ) {
...
AccountManager::Return<bool> AccountManager::add_view_account(
    ...
    const zinpass::AccountDAO account_dao;
    if (zinpass::DaoStatus ret_account_dao = account_dao.add(view_account);
        zinpass::DaoStatus::Success != ret_account_dao
        ) {
        return {false, "失败" + std::to_string(static_cast<int>(ret_account_dao))};
    }
...
AccountManager::Return<bool> AccountManager::update_account(
    const zinpass::ViewAccount& view_account,
    const std::string& plaintext_pwd,
    const std::string& main_key){
    // 0. 定义变量
    const std::string current_date_time = zinpass::DateTime::getCurrentDateTime();
    zinpass::AccountDAO account_dao;
    zinpass::ViewAccount new_view_account(view_account);
    ...
    if (const zinpass::DaoStatus ret_account_dao_2 = account_dao.update(new_view_account);
        zinpass::DaoStatus::Success != ret_account_dao_2) {
        return {false, "更新失败：" + std::to_string(static_cast<int>(ret_account_dao_2))};
    }
    return {true, "成功"};
}

AccountManager::Return<bool> AccountManager::update_account_base_info(const zinpass::ViewAccount& view_account){
    // 0. 定义变量
    const std::string current_date_time = zinpass::DateTime::getCurrentDateTime();
    const zinpass::AccountDAO account_dao;
    ...
    if (const zinpass::DaoStatus ret_account_dao_2 = account_dao.update_main_properties(view_account);   // 忽略 encrypted_pwd 和 iv
        zinpass::DaoStatus::Success != ret_account_dao_2) {
        return {false, "更新失败：" + std::to_string(static_cast<int>(ret_account_dao_2))};
    }
    return {true, "成功"};
...
AccountManager::Return<bool> AccountManager::update_account_password(
    ...
    if (const zinpass::DaoStatus ret_account_dao_2 = account_dao.update_password(account_id, sys_user_id, ciphertext, initialization_vector, current_date_time);
        zinpass::DaoStatus::Success != ret_account_dao_2) {
        return {false, "更新失败：" + std::to_string(static_cast<int>(ret_account_dao_2))};
    }
    return {true, "成功"};
...
bool AccountManager::re_encrypt_password(
    ...
    // 1. 解密旧密码
    if (const zinpass::DaoStatus ret_find1 = account_dao.findEncryptedPwdAndIv(account_id, sys_user_id, old_ec_pwd, old_iv);
        zinpass::DaoStatus::Success != ret_find1) {
        return false;
    }
    std::string plaintext_pwd;                 // 解密后的明文密码
    const std::vector<unsigned char> old_real_key = zinpass::PwdUtils::hexStringToKey(zinpass::PwdUtils::calculateMD5(old_main_key));
    if (const bool ret_decrypt = zinpass::PwdUtils::decryptTextByAES128CTR(old_ec_pwd, old_iv, old_real_key, plaintext_pwd);
        !ret_decrypt) {
        return false;    // 解密失败
        }

    // 2. 加密新密码
    const std::vector<unsigned char> new_real_key = zinpass::PwdUtils::hexStringToKey(zinpass::PwdUtils::calculateMD5(new_main_key));
    std::vector<unsigned char> new_ec_pwd;
    std::vector<unsigned char> new_iv;
    if (const bool ret_encrypt = zinpass::PwdUtils::encryptTextByAES128CTR(plaintext_pwd, new_real_key, new_ec_pwd, new_iv);
        !ret_encrypt) {
        return false;    // 加密失败
        }
    zinpass::PwdUtils::encryptTextByAES128CTR(plaintext_pwd, new_real_key, new_ec_pwd, new_iv);

    // 3. 更新数据库中密码和初始化向量
    if (const zinpass::DaoStatus ret_update = account_dao.update_password(account_id, sys_user_id, new_ec_pwd, new_iv, now);
        zinpass::DaoStatus::Success != ret_update) {
        return false;    // 更新失败
    }
...
AccountManager::Return<bool> AccountManager::delete_account(const std::string& account_id) {
    try {
        const zinpass::AccountDAO account_dao;
        const zinpass::DaoStatus ret_account_dao = account_dao.remove(account_id);
        if (zinpass::DaoStatus::Success != ret_account_dao) {
            return {false, "删除失败：" + std::to_string(static_cast<int>(ret_account_dao))};
        }
        return {true, "删除成功"};
    } catch (const std::invalid_argument& e) {
        zinpass::LogManager::add_log(std::string("[ERROR] Invalid argument when deleting account: ") + e.what());
    } catch (const std::exception& e) {
        zinpass::LogManager::add_log(std::string("[ERROR] Exception when deleting account: ") + e.what());
    }
    return {false, "删除失败"};
}
