...
MobilePhoneManager::Return<std::vector<std::string>> MobilePhoneManager::get_phone_options(const std::string& sys_user_id) {
    ...
    std::string message;
    const zinpass::MobilePhoneDAO mobile_phone_dao;
    if (const zinpass::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_list(sys_user_id, phone_numbers);
        zinpass::DaoStatus::Success != ret_mobile_phone_dao ) {
        message = "发生错误";
    }else {
        message = "成功";
    }
...
MobilePhoneManager::Return<std::vector<zinpass::MobilePhone>> MobilePhoneManager::get_mobile_phones(const std::string& sys_user_id, const std::string& telecom_operator, const std::string& phone_number) {
    std::vector<zinpass::MobilePhone> phones;
    std::string message;
    const zinpass::MobilePhoneDAO mobile_phone_dao;
    if (const zinpass::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_all(sys_user_id, telecom_operator, phone_number, phones);
        zinpass::DaoStatus::Success != ret_mobile_phone_dao) {
        message = "发生错误";
    }else {
        message = "成功";
...
MobilePhoneManager::Return<zinpass::MobilePhone> MobilePhoneManager::get_phone_by_owner_and_id(const std::string& sys_user_id, const int phone_id) {
    const zinpass::MobilePhone phone;
    zinpass::MobilePhone temp_phone;

    const zinpass::MobilePhoneDAO mobile_phone_dao;
    if (const zinpass::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_by_id(phone_id, temp_phone);
        zinpass::DaoStatus::Success != ret_mobile_phone_dao) {
        return {phone, "发生错误"};
    }
...
MobilePhoneManager::Return<zinpass::MobilePhone> MobilePhoneManager::get_phone_by_id(const int phone_id) {
    zinpass::MobilePhone phone;
    const zinpass::MobilePhoneDAO mobile_phone_dao;
    if (const zinpass::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_by_id(phone_id, phone);
        zinpass::DaoStatus::Success != ret_mobile_phone_dao) {
        return {phone, "发生错误"};
    }
    return {phone, "成功"};
...
MobilePhoneManager::Return<bool> MobilePhoneManager::add_mobile_phone(
    ...
    if (
        const zinpass::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.add(new_phone);
        zinpass::DaoStatus::Success != ret_mobile_phone_dao) {
        message = "发生了错误";
        ret = false;
    }else {
...
MobilePhoneManager::Return<bool> MobilePhoneManager::update_mobile_phone(
    ...
    const zinpass::DaoStatus ret_update = mobile_phone_dao.update(id,owner_id,
...
std::tuple<bool, std::string, int> MobilePhoneManager::delete_mobile_phone(const int id, const std::string& user_id, const int mode, const int replace_phone_id) {
    ...
    if (ref_count == 0) { // 被引次数为 0，任何模式均可以删除
        if (const zinpass::DaoStatus ret_status = mobile_phone_dao.remove(id, user_id);
    ...
    switch (mode) {
        ...
        case 1: { // m1 - 若存在子表引用行，批量修改被引用记录的手机号为指定的手机号
            if (const auto ret_rr = mobile_phone_dao.replace_and_remove(id, user_id, replace_phone_id, now);
                zinpass::DaoStatus::Success == ret_rr) {
        ...
        case 3: { // m3 - 若存在子表引用行，级联删除所有子表引用记录
            if (const zinpass::DaoStatus ret_status = mobile_phone_dao.remove_cascade(id, user_id);
                zinpass::DaoStatus::Success != ret_status) {
                return {false, "执行删除操作失败", ref_count};
            }
            return {true, "成功", ref_count};
