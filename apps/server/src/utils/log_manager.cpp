...
void LogManager::log_worker() {
    std::string run_log_path = zinpass::AppConfig::instance().get_log_path();   // 获取运行日志路径
    ...
    if (!log_file.is_open()) {
        ...
        zinpass::AppConfig::instance().set_log_path(run_log_path);
        log_file.open(run_log_path, std::ios::app);

        if(!log_file.is_open()){
            // std::cerr << << zinpass::DateTime::getCurrentDateTime() << " [0x" << std::this_thread::get_id() << "] " << "Failed to open all log files. Subsequent logs will be lost." << std::endl;
            return;
        }
    ...
    while (true) {
        ...
        while (!log_queue_.empty()) {
            ...
            std::stringstream ss;
            ss << zinpass::DateTime::getCurrentDateTime()
