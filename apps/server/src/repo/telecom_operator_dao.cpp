...
TelecomOperatorDAO::TelecomOperatorDAO() : pool_(ConnectionPool::instance()){
}
...
DaoStatus TelecomOperatorDAO::findList(std::vector<std::string>& telecom_operators) const {
    ...
    zinpass::LogManager::add_log(std::string("[OK] Number of Telecom Operator records found: ") + std::to_string(telecom_operators.size()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
