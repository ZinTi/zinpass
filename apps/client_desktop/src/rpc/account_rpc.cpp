...
namespace zinpass {
    ...
    AccountRPC::Return<zinpass::ViewAccount> AccountRPC::list_account_by_id(const std::string& session_id, const std::string& account_id){
        ...
        if(!status.ok()){
            zinpass::ViewAccount empty_view_account;
            return {empty_view_account, response.message()};
        }
        zinpass::ViewAccount view_acc;
    ...
    AccountRPC::Return<std::vector<zinpass::ViewAccount>> AccountRPC::list_accounts(
        const std::string& session_id,
        const std::string& provider_name,
        const std::string& platform_name,
        const std::string& username,
        ...
        grpc::Status status = stub_->ListAccounts(&context, request, &response);
        if(!status.ok()){
            std::vector<zinpass::ViewAccount> empty_vector;
            return {empty_vector, response.message()};
        }
