...
ConnectionPool& ConnectionPool::instance() {
    if (!instance_) {
        zinpass::LogManager::add_log("[ERROR] ConnectionPool not initialized");
...
ConnectionPool::ConnectionPool(const std::string& db_path, const size_t pool_size)
    ...
    for (size_t i = 0; i < pool_size_; ++i) {
        ...
        // zinpass::LogManager::add_log("[OK] Opening database connection");
        const int rc = sqlite3_open(db_path_.c_str(), &conn);
        if (rc != SQLITE_OK) {
            sqlite3_close(conn);
            const std::string err_msg(sqlite3_errmsg(conn));
            zinpass::LogManager::add_log(std::string("[ERROR] Failed to open database: ") + err_msg);
        ...
        if (pragma_rc != SQLITE_OK) {
            ...
            sqlite3_free(errMsg);
            sqlite3_close(conn);
            zinpass::LogManager::add_log("[ERROR] Failed to enable foreign keys: " + pragma_err);
...
ConnectionPool::~ConnectionPool() {
    ...
    while (!available_connections_.empty()) {
        ...
        // zinpass::LogManager::add_log("[OK] Closing database connection");
        sqlite3_close(conn);
...
sqlite3* ConnectionPool::get_connection() {
    // zinpass::LogManager::add_log("[OK] get_connection()");
    std::unique_lock<std::mutex> lock(mutex_);
...
void ConnectionPool::release_connection(sqlite3* conn) {
    // zinpass::LogManager::add_log("[OK] release_connection(conn)");
    std::unique_lock<std::mutex> lock(mutex_);
    available_connections_.push(conn);
    lock.unlock();
