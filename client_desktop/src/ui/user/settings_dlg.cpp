...
void SettingsDlg::setup_ui() {
    ...
    const std::string ip_addr = zinpass::state::ClientConfig::instance().get_server_ipv4();
    const std::string port = std::to_string(zinpass::state::ClientConfig::instance().get_server_port());
    e_ip_address_ = new QLineEdit(this);
    e_ip_address_->setPlaceholderText(QString::fromStdString(ip_addr));
    e_ip_address_->setText(QString::fromStdString(ip_addr));
    e_port_ = new QLineEdit(this);
    e_port_->setValidator(new QIntValidator(1, 65535, this));
    e_port_->setPlaceholderText("0-65535");
    e_port_->setText(QString::fromStdString(port));
