...
gRPCClientService::~gRPCClientService() {
    ...
    zinpass::LogManager::add_log("[MESSAGE] gRPCClientService Destructor");
}

void gRPCClientService::run() const {
    zinpass::LogManager::add_log("[MESSAGE] Server listening on " + this->address_and_port_ );
    ...
    if (!server_) {
        zinpass::LogManager::add_log("[ERROR] Failed to start server on " + address_and_port_);
        return;
    }
    ...
    zinpass::LogManager::add_log("[MESSAGE] Server stopped on " + address_and_port_);
}
