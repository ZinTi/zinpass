...
SystemUserDAO::SystemUserDAO() : pool_(ConnectionPool::instance()) {
}

SystemUserDAO::~SystemUserDAO() = default;

DaoStatus SystemUserDAO::findById(const std::string& id, zinpass::SystemUser& sys_user) const {
    ...
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        zinpass::LogManager::add_log(std::string("[NOT FOUND] User not found with the given ID in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
    ...
    zinpass::LogManager::add_log("[OK] User found.");
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}

DaoStatus SystemUserDAO::findByUsername(const std::string& username, zinpass::SystemUser& sys_user) const {
    sqlite3* conn = pool_.get_connection();
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        zinpass::LogManager::add_log(std::string("[NOT FOUND] User not found with the given username in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::NotFound;
    }
...
DaoStatus SystemUserDAO::findUsernames(std::vector<std::string>& usernames) const {
    ...
    zinpass::LogManager::add_log(std::string("[OK] Number of Username records found: ") + std::to_string(usernames.size()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}

DaoStatus SystemUserDAO::add(const zinpass::SystemUser& sys_user) const {
    sqlite3* conn = pool_.get_connection();
    ...
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        zinpass::LogManager::add_log(std::string("[ERROR] Failed to insert system_user record: ") + std::string(sqlite3_errmsg(conn)));
        status = DaoStatus::DuplicateKey;
    }else {
        zinpass::LogManager::add_log(std::string("[OK] New user created successfully: ") + sys_user.getId());
...
DaoStatus SystemUserDAO::update(const zinpass::SystemUser& sys_user) const {
    sqlite3* conn = pool_.get_connection();
    ...
    zinpass::LogManager::add_log(std::string("[OK] User updated successfully: ") + sys_user.getId());
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus SystemUserDAO::remove(const std::string& id) const {
    ...
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        ...
        zinpass::LogManager::add_log(std::string("[OK] User removed successfully: ") + id);
        status = DaoStatus::Success;
    }
