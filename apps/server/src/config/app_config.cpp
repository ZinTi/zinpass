...
bool AppConfig::read_config_from_file(std::string& msg) {
    // 构建配置文件路径：可执行文件所在目录的父目录 / server_config.yml
    std::filesystem::path conf_path;
    try {
        std::filesystem::path executable_path = zinpass::get_executable_path();
        conf_path = executable_path.parent_path() / "server_config.yml";
    } catch (const std::exception& e) {
