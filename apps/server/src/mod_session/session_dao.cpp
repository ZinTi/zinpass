...
namespace zinpass {
    SessionDAO::SessionDAO() : pool_(zinpass::ConnectionPool::instance()) {
    }

    SessionDAO::~SessionDAO() = default;

    zinpass::DaoStatus SessionDAO::find_by_id(const std::string& id, zinpass::Session& session) const {
        ...
        if (SQLITE_OK != sqlite3_prepare(conn, sql.c_str(), -1, &stmt, nullptr)) {
            ...
            return zinpass::DaoStatus::InvalidData;
        }
        sqlite3_bind_text(stmt, 1, id.c_str(), static_cast<int>(id.size()), SQLITE_TRANSIENT);
        zinpass::SQLDebug::log_sql(stmt, true);
        if (SQLITE_ROW != sqlite3_step(stmt)) {
            zinpass::LogManager::add_log(std::string("[NOT FOUND] Session not found with the given ID in the database."));
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
            return zinpass::DaoStatus::NotFound;
        }
        ...
        // zinpass::LogManager::add_log("[OK] Session found.");
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return zinpass::DaoStatus::Success;
    }
    ...
    zinpass::DaoStatus SessionDAO::add(const zinpass::Session& session) const {
        sqlite3* conn = pool_.get_connection();
        ...
        zinpass::SQLDebug::log_sql(stmt, true);
        zinpass::DaoStatus status;
        const int result = sqlite3_step(stmt);
        if (SQLITE_DONE == result) {
            status = zinpass::DaoStatus::Success;
            zinpass::LogManager::add_log("[OK] New Session created: " + session.getId());
        } else {
            const char* errMsg = sqlite3_errmsg(conn); // 在释放资源前获取错误信息（避免连接释放后丢失）
            switch (result) {
            case SQLITE_CONSTRAINT: {
                status = zinpass::DaoStatus::DuplicateKey; // 约束冲突（唯一键、外键、非空约束等）
            ...
            case SQLITE_BUSY: {
                status = zinpass::DaoStatus::DatabaseBusy; // 数据库被锁定（如其他连接持有写锁）
                zinpass::LogManager::add_log(
            ...
            case SQLITE_READONLY: {
                status = zinpass::DaoStatus::ReadOnlyError; // 数据库文件只读或权限不足
                zinpass::LogManager::add_log(
            ...
            case SQLITE_MISUSE: {
                status = zinpass::DaoStatus::ConfigurationError; // API使用错误（如未正确关闭stmt）
                zinpass::LogManager::add_log(
            ...
            case SQLITE_ERROR:
            default: {
                status = zinpass::DaoStatus::GenericError; // 通用错误（如数据类型转换失败、SQL语法错误等）
                zinpass::LogManager::add_log(
    ...
    zinpass::DaoStatus SessionDAO::update(const zinpass::Session& session) const {
        sqlite3* conn = pool_.get_connection();
        ...
        if (SQLITE_DONE != sqlite3_step(stmt)) {
            zinpass::LogManager::add_log(std::string("[ERROR] Failed to update session: ") + std::string(sqlite3_errmsg(conn)));
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
            return zinpass::DaoStatus::GenericError;
        }
        zinpass::LogManager::add_log(std::string("[OK] Session updated successfully: ") + session.getId());
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return zinpass::DaoStatus::Success;
    }

    zinpass::DaoStatus SessionDAO::remove(const std::string& id) const {
        sqlite3* conn = pool_.get_connection();
        ...
        zinpass::LogManager::add_log(std::string("[OK] Session removed successfully: ") + id);
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return zinpass::DaoStatus::Success;
    }
    ...
    zinpass::DaoStatus SessionDAO::update_expiration(const std::string& id, const std::string& expires_date_time) const{
        sqlite3* conn = pool_.get_connection();
        ...
        if (SQLITE_DONE != sqlite3_step(stmt)) {
            zinpass::LogManager::add_log(std::string("[ERROR] Failed to update session expiration: ") + std::string(sqlite3_errmsg(conn)));
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
            return zinpass::DaoStatus::GenericError;
        }
        zinpass::LogManager::add_log(std::string("[OK] Session expiration updated successfully: ") + id);
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return zinpass::DaoStatus::Success;
    }
