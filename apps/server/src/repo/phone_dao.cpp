...
MobilePhoneDAO::MobilePhoneDAO() : pool_(ConnectionPool::instance()) {
}

MobilePhoneDAO::~MobilePhoneDAO() = default;

zinpass::MobilePhone MobilePhoneDAO::MobilePhoneFromStatement(sqlite3_stmt* stmt) {
    zinpass::MobilePhone mobile_phone;

    if (SQLITE_NULL != sqlite3_column_type(stmt, 0)) {
        mobile_phone.setId(sqlite3_column_int(stmt, 0));
    }else {
        zinpass::LogManager::add_log("[WARNING] Severe Warning: The primary key of the phone record is NULL, which may indicate an error. Resolution: Assign a default value of 0 to the variable.");
...
int MobilePhoneDAO::generate_id() const {
    ...
    if (SQLITE_OK != sqlite3_prepare_v2(conn, sql_find_unused_id.c_str(), static_cast<int>(sql_find_unused_id.size()), &stmt, nullptr)) {
        zinpass::LogManager::add_log(std::string("Prepare failed: ") + std::string(sqlite3_errmsg(conn)));
...
DaoStatus MobilePhoneDAO::find_by_id(const int id, zinpass::MobilePhone& mobile_phone) const {
    sqlite3* conn = pool_.get_connection();
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        zinpass::LogManager::add_log(std::string("[NOT FOUND] Phone not found with the given ID in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
    ...
    zinpass::LogManager::add_log("[OK] Mobile phone found.");
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}

DaoStatus MobilePhoneDAO::find_by_number(const std::string& phone_number, zinpass::MobilePhone& mobile_phone) const {
    sqlite3* conn = pool_.get_connection();
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        zinpass::LogManager::add_log(std::string("[NOT FOUND] Phone not found with the given phone_number in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::NotFound;
    }
...
int MobilePhoneDAO::find_id_by_number(const std::string& phone_number) const {
    ...
    zinpass::LogManager::add_log("[OK] Phone id found.");
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return id;
}
...
DaoStatus MobilePhoneDAO::find_all(
    ...
    std::vector<zinpass::MobilePhone>& mobile_phones) const{
    sqlite3* conn = pool_.get_connection();
    ...
    zinpass::LogManager::add_log("[OK] Phone found.");
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus MobilePhoneDAO::find_list(const std::string& sys_user_id, std::vector<std::string>& phone_numbers) const{
    ...
    zinpass::LogManager::add_log(std::string("[OK] Number of MobilePhone records found: ") + std::to_string(phone_numbers.size()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus MobilePhoneDAO::add(const zinpass::MobilePhone& mobile_phone) const {
    sqlite3* conn = pool_.get_connection();
    ...
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        zinpass::LogManager::add_log(std::string("[ERROR] Failed to insert mobile_phone record: ") + std::string(sqlite3_errmsg(conn)));
        status = DaoStatus::DuplicateKey;
    }else {
        zinpass::LogManager::add_log(std::string("[OK] New mobile phone created successfully: ") + std::to_string(primaryKey));
...
DaoStatus MobilePhoneDAO::update(
    ...
    zinpass::LogManager::add_log(std::string("[OK] Phone updated successfully: ") + std::to_string(id));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus MobilePhoneDAO::remove(const int id, const std::string& sys_user_id) const {
    ...
    zinpass::LogManager::add_log(std::string("[OK] Phone removed successfully: ") + std::to_string(id));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
[[nodiscard]] DaoStatus MobilePhoneDAO::replace_and_remove(
    ...
    zinpass::LogManager::add_log(std::string("[OK] Foreign key of table account updated successfully: ") + std::to_string(id));
    sqlite3_finalize(stmt1);
...
DaoStatus MobilePhoneDAO::remove_cascade(const int id, const std::string& sys_user_id) const {
    ...
    zinpass::LogManager::add_log(std::string("[OK] Account records removed successfully: ") + std::to_string(id));
    sqlite3_finalize(stmt1);
