...
namespace zinpass {
    ...
    PhoneRPC::Return<std::vector<zinpass::MobilePhone>> PhoneRPC::list_phones(const std::string& session_id, const std::string& telecom, const std::string& phone_number){
        // 1. construct request
        ...
        std::vector<zinpass::MobilePhone> mobile_phones;
        if (!status.ok()) {
    ...
    PhoneRPC::Return<zinpass::MobilePhone> PhoneRPC::find_phone_by_id(const std::string& session_id, const int phone_id){
        // 1. construct request
