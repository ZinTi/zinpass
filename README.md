...
- `zinpassd` 服务端，部署在 Linux 上为守护进程，负责后端数据处理，使用 gRPC 通信，使用 SQLite3 存储数据，使用 OpenSSL 加密数据（目前版本只加密密码字段）。
...
- 若你没有个人服务器，只希望本地运行，例如安装在 *Windows x86_64* 平台的电脑上使用，请选择下载 *Windows x86_64* 版本的服务端程序 `zinpassd.exe` 和客户端程序 `zinpass.exe`，按照推荐的目录结构安装部署。 
