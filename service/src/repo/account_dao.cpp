...
DaoStatus AccountDAO::findById(const std::string& id, models::ViewAccount& view_account) const {
    ...
    if (SQLITE_OK != sqlite3_prepare_v2(conn, sql.c_str(), -1, &stmt, nullptr)) {
        utils::LogManager::add_log(std::string("[ERROR] Failed to prepare statement: ") + std::string(sqlite3_errmsg(conn)));
    ...
    if (SQLITE_ROW == sqlite3_step(stmt)) {
        ...
        utils::LogManager::add_log("[OK] Account found.");
        status = DaoStatus::Success;
    } else {
        utils::LogManager::add_log(std::string("[NOT FOUND] Account not found with the given ID in the database."));
...
DaoStatus AccountDAO::find(const std::string& sys_user_id, std::vector<models::ViewAccount>& view_accounts) const {
    ...
    utils::LogManager::add_log(std::string("[OK] All accounts fetched."));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
...
DaoStatus AccountDAO::find(
    ...
    utils::LogManager::add_log("[OK] Accounts fetched based on the search criteria.");
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus AccountDAO::findEncryptedPwdAndIv(const std::string& id, const std::string& sys_user_id,
        ...
        SQLITE_ROW == stepResult)
    {
        ...
        utils::LogManager::add_log(std::string("[NOT FOUND] No rows found."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::NotFound;
    } else {
        utils::LogManager::add_log(std::string("[FAILED] Step failed: ") + std::string(sqlite3_errmsg(conn)));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
...
std::vector<std::string> AccountDAO::list_id(const std::string& sys_user_id) const {
    ...
    utils::LogManager::add_log(std::string("[OK] All accounts id fetched."));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return data_list;
}
...
DaoStatus AccountDAO::add(const models::Account& account) const {
    ...
    if ( SQLITE_DONE != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[ERROR] Failed to insert account record: ") + std::string(sqlite3_errmsg(conn)));
        status = DaoStatus::DuplicateKey;
    } else {
        utils::LogManager::add_log(std::string("[OK] Account inserted successfully."));
        status = DaoStatus::Success;
    }
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
...
DaoStatus AccountDAO::update(const models::ViewAccount& view_account) const {
    ...
    if (view_account.getEncryptedPwd().empty()) {
        utils::LogManager::add_log(std::string("[ERROR] Encrypted password is empty."));
        return DaoStatus::InvalidData;
    }
    ...
    if (view_account.getIv().empty()) {
        utils::LogManager::add_log(std::string("[ERROR] IV is empty."));
        return DaoStatus::InvalidData;
    }
    ...
    if (view_account.getId().empty()) {
        utils::LogManager::add_log(std::string("[INVALID DATA] 不合法主键。主键长度为0."));
        return DaoStatus::InvalidData;
    }
    ...
    if (view_account.getSysUserId().empty()) {
        utils::LogManager::add_log(std::string("[INVALID DATA] SysUserId is NULL."));
        return DaoStatus::InvalidData;
    }
    ...
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[ERROR] Failed to update record: ") + std::string(sqlite3_errmsg(conn)));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::GenericError;
    }

    utils::LogManager::add_log(std::string("[OK] Account updated successfully: ") + view_account.getId());
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus AccountDAO::update_main_properties(const models::ViewAccount& view_account) const {
    ...
    // utils::LogManager::add_log(std::string("[OK] Account updated successfully: ") + std::to_string(view_account.getId()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus AccountDAO::update_password(const std::string& id,
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("Failed to update record: ") + std::string(sqlite3_errmsg(conn)));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::GenericError;
    }

    utils::LogManager::add_log(std::string("[OK] Account updated successfully: ") + id);
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus AccountDAO::remove(const std::string& id) const {
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[ERROR] Failed to remove record: ") + std::string(sqlite3_errmsg(conn)));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::GenericError;
    }

    utils::LogManager::add_log(std::string("[OK] Account removed successfully: ") +id);
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus AccountDAO::findEmailList(
    ...
    utils::LogManager::add_log(std::string("[OK] Number of email records found: ") + std::to_string(email_addresses.size()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
std::string AccountDAO::getIdByEmailAddress(const std::string& email_address) const {
    ...
    sqlite3_stmt* stmt = nullptr;
    if (SQLITE_OK != sqlite3_prepare_v2(conn, sql.c_str(), -1, &stmt, nullptr)) {
        utils::LogManager::add_log(std::string("[ERROR] Failed to prepare statement: ") +
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[NOT FOUND] Email(Account) ID not found with the given emailAddress in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return "";
    }
...
DaoStatus AccountDAO::findEmailById(const std::string& id, models::ViewAccount& email) const {
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[NOT FOUND] Email(Account) not found with the given ID in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::NotFound;
    }
