...
namespace zinpass {
    ...
    int InitDatabase::executeSQL(sqlite3* conn, const std::string& sql) {
        ...
        if (
            ...
            SQLITE_OK != rc)
        {
            zinpass::LogManager::add_log(std::string("SQL error: ") + std::string(errMsg) + std::string("\n"));
            sqlite3_free(errMsg);
            return rc;
        }
        return 0;
    ...
    int InitDatabase::init() const {
        ...
        if (const int rc = sqlite3_open(this->db_path_.c_str(), &conn)){
            zinpass::LogManager::add_log(std::string("[ERROR] Failed to open database: ") + std::string(sqlite3_errmsg(conn)));
            sqlite3_close(conn);
            return rc;
        }
