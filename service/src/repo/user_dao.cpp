...
DaoStatus SystemUserDAO::findById(const std::string& id, models::SystemUser& sys_user) const {
    ...
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[NOT FOUND] User not found with the given ID in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
    ...
    utils::LogManager::add_log("[OK] User found.");
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
...
DaoStatus SystemUserDAO::findByUsername(const std::string& username, models::SystemUser& sys_user) const {
    ...
    SQLDebug::log_sql(stmt, true);
    if (SQLITE_ROW != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[NOT FOUND] User not found with the given username in the database."));
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return DaoStatus::NotFound;
    }
...
DaoStatus SystemUserDAO::findUsernames(std::vector<std::string>& usernames) const {
    ...
    utils::LogManager::add_log(std::string("[OK] Number of Username records found: ") + std::to_string(usernames.size()));
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus SystemUserDAO::add(const models::SystemUser& sys_user) const {
    ...
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        utils::LogManager::add_log(std::string("[ERROR] Failed to insert system_user record: ") + std::string(sqlite3_errmsg(conn)));
        status = DaoStatus::DuplicateKey;
    }else {
        utils::LogManager::add_log(std::string("[OK] New user created successfully: ") + sys_user.getId());
...
DaoStatus SystemUserDAO::update(const models::SystemUser& sys_user) const {
    ...
    utils::LogManager::add_log(std::string("[OK] User updated successfully: ") + sys_user.getId());
    sqlite3_finalize(stmt);
    pool_.release_connection(conn);
    return DaoStatus::Success;
}
...
DaoStatus SystemUserDAO::remove(const std::string& id) const {
    ...
    if (SQLITE_DONE != sqlite3_step(stmt)) {
        ...
        utils::LogManager::add_log(std::string("[OK] User removed successfully: ") + id);
        status = DaoStatus::Success;
    }
