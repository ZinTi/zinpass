...
void AccountFilterForm::setup_layout(){
    ...
    // 表单布局
    lyt_form_left_ = new QFormLayout(this);
    ...
    lyt_form_right_ = new QFormLayout(this);
...
void AccountFilterForm::initial_input_widgets() const {
    // 1. 定义变量
    const std::string session_id = zinpass::state::StateManager::instance().get_user_state().session_id;

    // 2. 获取数据：可选 phone_numbers、email_addresses、account_categories
    const auto channel = zinpass::state::ChannelManager::instance().get_channel();
